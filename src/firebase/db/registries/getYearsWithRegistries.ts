...
export const dbGetYearsWithRegistries: dbGetYearsWithRegistriesType = async (uid) => {
  try {
    const monthRegistriesCollection = collection(FIREBASE_DB, 'USERS', uid, 'REGISTRIES')
    const registriesSnapshot = await getDocs(monthRegistriesCollection)
    const years = registriesSnapshot.docs.map((doc) => doc.id)
    const now = new Date()
    const hasCurrentYear = years.find((year) => year === `${now.getFullYear()}`) !== undefined
