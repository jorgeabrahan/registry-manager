...
export const Transaction: React.FC<TransactionProps> = ({
  ...
  const handleRemoveTransaction = () => {
    showConfirmModal({
      ...
      )?.label?.toLowerCase()} ${transaction.name} con un monto de ${formatHNL(
        transaction.amount
      )}`,
      onConfirm: () => {
        onRemoveTransaction()
  ...
  return (
    <article className='flex flex-col border border-solid border-tundora-800 rounded-xl p-3 bg-tundora-950 min-h-[180px]'>
      ...
      {transaction.description?.length !== 0 && (
        <p className='text-dove-gray-300 text-sm mb-2 truncate-2'>
      ...
      <footer className='mt-auto font-mono'>
        {transaction.type === TransactionTypes.credit && (
          <>
            <p className='text-sm'>
              <span className='text-dove-gray-200 text-xs font-semibold'>
                Recibido:
              </span>{' '}
